              ...
              <div className="space-y-2">
                {realtimeOrders.slice(0, 3).map((order, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                    <span className="text-sm">New order in your area</span>
          ...
          <TabsContent value="products">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {products?.filter(product => 
                product.targetUsers?.includes('retail_user') && product.isActive
              ).map((product) => (
                <ProductCard
                  key={product.id}
                  product={product}
          ...
          <>
            ...
            <div className="bg-gray-50 p-3 rounded">
              ...
              <div className="flex justify-between font-medium">
                <span>Total:</span>
                <span>₹{(selectedVariant.price * quantity).toLocaleString()}</span>
              </div>
            </div>
