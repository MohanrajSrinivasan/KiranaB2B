...
export async function seedHybridStorage() {
  try {
    // Check if data already exists
    const existingUsers = await storage.getAllProducts();
    ...
    const kiranaUser = await storage.createUser({
    ...
    const retailUser = await storage.createUser({
    ...
    if (savedProducts.length > 0) {
      const sampleOrder1 = await storage.createOrder({
        userId: kiranaUser._id || kiranaUser.id,
        items: [
          {
            productId: savedProducts[0]._id || savedProducts[0].id,
            quantity: 5,
            price: 120,
            name: 'Basmati Rice Premium'
          },
          {
            productId: savedProducts[2]._id || savedProducts[2].id,
            quantity: 3,
            price: 85,
            name: 'Toor Dal (Arhar)'
          }
      ...
      const sampleOrder2 = await storage.createOrder({
        userId: retailUser._id || retailUser.id,
        items: [
          {
            productId: savedProducts[1]._id || savedProducts[1].id,
            quantity: 2,
            price: 45,
            name: 'Organic Wheat Flour'
          },
          {
            productId: savedProducts[5]._id || savedProducts[5].id,
            quantity: 1,
            price: 450,
            name: 'Pure Desi Ghee'
          }
