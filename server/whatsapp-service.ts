...
export class WhatsAppService {
  ...
  async sendOrderConfirmation(toNumber: string, orderDetails: any) {
    try {
      if (!this.client) {
...
Items:
${orderDetails.items.map((item: any) => `- ${item.productName} (${item.label}) x${item.quantity}`).join('\n')}
...
Thank you for your order! 🙏`;
  ...
  async sendOrderStatusUpdate(toNumber: string, orderDetails: any) {
    try {
      if (!this.client) {
        console.warn('Twilio not configured - skipping WhatsApp message');
        return;
...
Track your order status anytime on our platform.`;
  ...
  async sendLowStockAlert(toNumber: string, products: any[]) {
    try {
      if (!this.client) {
        console.warn('Twilio not configured - skipping WhatsApp message');
        return;
